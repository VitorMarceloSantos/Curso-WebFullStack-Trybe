...
const name =  Joi.string().min(3).max(10).required().messages({
...
const country = Joi.string().min(4).required().messages({
	'string.min': 'Minímo de 4 caracteres',
	'string.empty': ' Campo obrigatório',
}).custom(transformCaseSensitive);
const destination = Joi.string().min(3).required().messages({
	'string.min': 'Minímo de 3 caracteres',
	'string.empty': ' Campo obrigatório',
}).custom(transformCaseSensitive);
